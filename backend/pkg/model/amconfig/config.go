...
// remove custom Secret Marshal/Unmarshal Methods
// replace secretURL with URL
...
func (c *Config) UnmarshalYAML(unmarshal func(interface{}) error) error {
	...
	for _, rcv := range c.Receivers {
		...
		for _, ogc := range rcv.OpsGenieConfigs {
			...
			if !strings.HasSuffix(ogc.APIURL.uURl.Path, "/") {
				ogc.APIURL.uURl.Path += "/"
			}
		...
		for _, wcc := range rcv.WechatConfigs {
			...
			if !strings.HasSuffix(wcc.APIURL.uURl.Path, "/") {
				wcc.APIURL.uURl.Path += "/"
			}
		}
		for _, voc := range rcv.VictorOpsConfigs {
			...
			if !strings.HasSuffix(voc.APIURL.uURl.Path, "/") {
				voc.APIURL.uURl.Path += "/"
			}
...
func DefaultGlobalConfig() GlobalConfig {
	defaultHTTPConfig := httpconfig.DefaultHTTPClientConfig
	return GlobalConfig{
