...
func (c *ProxyConfig) Validate() error {
	if len(c.ProxyConnectHeader) > 0 && (!c.ProxyFromEnvironment && (c.ProxyURL.uURL == nil || c.ProxyURL.uURL.String() == "")) {
		return fmt.Errorf("if proxy_connect_header is configured, proxy_url or proxy_from_environment must also be configured")
	}
	if c.ProxyFromEnvironment && c.ProxyURL.uURL != nil && c.ProxyURL.uURL.String() != "" {
	...
	if c.ProxyURL.uURL == nil && c.NoProxy != "" {
...
func (c *ProxyConfig) Proxy() (fn func(*http.Request) (*url.URL, error)) {
	...
	if c.ProxyURL.uURL != nil && c.ProxyURL.uURL.String() != "" {
		if c.NoProxy == "" {
			c.proxyFunc = http.ProxyURL(c.ProxyURL.uURL)
			return
		}
		proxy := &httpproxy.Config{
			HTTPProxy:  c.ProxyURL.uURL.String(),
			HTTPSProxy: c.ProxyURL.uURL.String(),
			NoProxy:    c.NoProxy,
		}
