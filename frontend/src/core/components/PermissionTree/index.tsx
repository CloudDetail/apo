...
function PermissionTree({
  ...
  const fetchSubjectPermissions = async () => {
    ...
    try {
      ...
      const initialCheckedKeys = (subjectPermissions || []).map(
        (permission: any) => permission.featureId,
      )
      if (!isControlled) {
  ...
  const actionButtons = (
    <Flex align="center" justify="flex-end" className="w-full" style={actionStyle}>
      <Button type="primary" className="m-2" onClick={handleSelectAll} icon={<BsCheckAll />}>
        {t('selectAll')}
      </Button>
      {!isControlled && (
        <Button type="primary" className="m-2" onClick={() => onSave?.(checkedKeys)}>
          {t('save')}
        </Button>
      )}
    </Flex>
