...
const MarkdownRender = ({ content, theme }: { content: string; theme: 'light' | 'dark' }) => {
  const components = {
    // Headers with icons
    h1: ({ children }: any) => (
      <h1 className="text-4xl font-bold mb-6 text-gray-900 dark:text-white flex items-center gap-3 group">
        <Hash className="w-8 h-8 text-blue-500 group-hover:text-blue-600 transition-colors" />
        {children}
      </h1>
    ),
    h2: ({ children }: any) => (
      <h2 className="text-3xl font-semibold mb-4 mt-8 text-gray-800 dark:text-gray-100 flex items-center gap-2 group">
        <Hash className="w-6 h-6 text-blue-500 group-hover:text-blue-600 transition-colors" />
        {children}
      </h2>
    ),
    h3: ({ children }: any) => (
      <h3 className="text-2xl font-semibold mb-3 mt-6 text-gray-800 dark:text-gray-100 flex items-center gap-2 group">
    ...
    blockquote: ({ children }: any) => (
      <blockquote className="my-6 border-l-4 border-blue-500 bg-blue-50 dark:bg-blue-900/20 p-4 rounded-r-lg">
        ...
        <div className="text-gray-700 dark:text-gray-300 italic leading-relaxed">{children}</div>
      </blockquote>
    ),
    ...
    li: ({ children, ...props }: any) => {
      ...
      if (isTaskList) {
        return (
          <li className="flex items-start gap-3 text-gray-700 dark:text-gray-300 list-none">
            {children}
          </li>
        )
      }
    ...
    td: ({ children }: any) => (
      <td className="px-4 py-3 text-gray-700 dark:text-gray-300 border-b border-gray-100 dark:border-gray-600">
        {children}
      </td>
    ),
